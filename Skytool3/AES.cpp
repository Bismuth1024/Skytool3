...
void AES::EncryptBlock(uint8_t *in, uint8_t *out, uint8_t *roundKeys)
{
  ...
  for (round = 1; round <= N_r - 1; round++)
  {
    SubBytes(state);
    ShiftRows(state);
    MixColumns(state);
    AddRoundKey(state, roundKeys + round * 4 * N_b);
  }

  SubBytes(state);
  ShiftRows(state);
  AddRoundKey(state, roundKeys + N_r * 4 * N_b);
...
{
  uint8_t *tmp = new uint8_t[N_b];
  for (int j = 0; j < N_b; j++) {
    tmp[j] = state[i][(j + n) % N_b];
  }
  memcpy(state[i], tmp, N_b * sizeof(uint8_t));
...
void AES::KeyExpansion(uint8_t *key, uint8_t *w)
{
  ...
  while (i < 4 * N_b * (N_r + 1))
  {
    ...
    else if (N_k > 6 && i / 4 % N_k == 4)
    {
      SubWord(temp);
    }

    w[i + 0] = w[i - 4 * N_k] ^ temp[0];
    w[i + 1] = w[i + 1 - 4 * N_k] ^ temp[1];
    w[i + 2] = w[i + 2 - 4 * N_k] ^ temp[2];
    w[i + 3] = w[i + 3 - 4 * N_k] ^ temp[3];
    i += 4;
  }
...
{
  ShiftRow(state, 1, N_b - 1);
  ShiftRow(state, 2, N_b - 2);
  ShiftRow(state, 3, N_b - 3);
}
